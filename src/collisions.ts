...
export function toCircle(pill: Box, point: Point, servicePill: Box) {
...
export function circleWithParamPointCollision(circle: Box, x: number, y: number) {
    let dx = circle.x - x
    let dy = circle.y - y
    return dx * dx + dy * dy < circle.halfWidth * circle.halfWidth
}

export function pointWithParamBoxCollision(point: Point, x: number, y: number, width: number, height: number) {
    return point.x >= x && point.x < x + width && point.y >= y && point.y < y + height
}


export function pointWithCircleCollision(point: Point, circle: Box) {
    let dx = circle.x - point.x
    let dy = circle.y - point.y
    return dx * dx + dy * dy < circle.halfWidth * circle.halfWidth
}

export function pointWithBoxCollision(point: Point, box: Box) {
    return point.x >= box.left && point.x < box.right && point.y >= box.top && point.y < box.bottom
}

export function pointWithPillCollision(point: Point, pill: Box) {
    return pointWithCircleCollision(point, toCircle(pill, point, serviceSprite1))
}


export function circleWithCircleCollision(circle1: Box, circle2: Box) {
...
export function circleWithBoxCollision(circle: Box, box: Box) {
...
export function circleWithPillCollision(circle: Box, pill: Box) {
...
export function pillWithPillCollision(pill1: Box, pill2: Box) {
