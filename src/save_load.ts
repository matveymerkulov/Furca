...
export function arrayToString(array: any[], columns: number = undefined, padding = 0) {
    let text = "["
    addIndent()
...
export function booleanArrayToString(array: boolean[]) {
    let text = ""
    addIndent()
...
export function loadTextures(data: string, func: () => void) {
    initParser(data)
    getSymbol("=")
...
export function projectFromText(data: string) {
    initParser(data)
    getSymbol("(")
    getSymbol("{")
