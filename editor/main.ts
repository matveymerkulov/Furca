...
import {max} from "../../RuWebQuest 2/src/functions.js"
...
export function showAll()  {
    ...
    world.processSprites((sprite: Sprite) => {
...
project.init = () => {
    ...
    for(let item of document.getElementsByClassName("cancel") as HTMLButtonElement) {
        item.onclick = () => {
            hideWindow()
            deleteCurrentDrag()
        }
    ...
    if(saveKey.wasPressed) {
        ...
        if(electron !== undefined) {
            electron.saveDialog('showSaveDialog', {}).then(result => {
                ...
                electron.saveFile(result.filePath, projectToText())
            })
        } else {
