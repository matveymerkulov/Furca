...
export function initUpdate() {
    ...
    function initLevel(num: number) {
        ...
        if(level.value > 1) score.increment(levelBonus)
    }
    ...
    function createAsteroid(type: Object, angle = 0) {
    ...
    function removeAsteroid(asteroid: AngularSprite) {
        score.increment(asteroid.type.score)
        asteroids.remove(asteroid)
    }

    function onAsteroidHit(asteroid: Asteroid, bullet) {
    ...
    project.update = () => {
        ...
        if(currentState === state.alive) {
            ...
            flameSprite.visible = forward.keyIsDown
