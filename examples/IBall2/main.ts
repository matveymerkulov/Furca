...
project.init = () => {
    ...
    tileSet.setCollisionFromArray(new Sprite(undefined, 0.5, 0.5, 1.0, 1.0, ShapeType.circle)
    ...
    function tileCollision(shape: Sprite, tileNum: number, x: number, y: number) {
        switch (tileNum) {
            case keyTile:
            case bombTile:
            case figureTile:
    ...
    let panels = new Layer<VectorSprite>()
    ...
    function horizontalMovement(object: VectorSprite, leftKey: Key, rightKey: Key, acceleration: number, maxAcceleration: number) {
        if(leftKey.isDown) {
    ...
    project.update = () => {
        ...
        tiles.collisionWithSprite(player, (shape: Sprite, tileNum: number, x: number, y: number) => {
        ...
        for(let panel of panels.items) {
            ...
            tiles.collisionWithSprite(panel, (shape: Sprite) => {
